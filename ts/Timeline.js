...
class TimelineBase {
    constructor(option) {
        var _a;
        this.allElements = [];
        this.colorPack = ["#008080", "#43e1b0", "#f8b30f", "#f09204", "#ed5e80", "#b13450", "#830a3a", "#842cbc", "#510a7d", "#58a9ff", "#095db8", "#fc7272", "#5c0000",];
        this.dotLocations = [0];
        this.option = option;
        // Create new Element
        ...
        (_a = this.getRootElement()) === null || _a === void 0 ? void 0 : _a.appendChild(this.mainElement);
    }
    createElement(el, index, offset) {
        //  check right or left ########### right = true ######### left = false
        var width = Number(this.mainElement.style.width.replace("px", ""));
...
class Timeline extends TimelineBase {
    ...
    mode_Arrow(width, offset, reverse) {
    ...
    mode_Inline_Classic(width, offset) {
        this.mainElement.style.height = `${((this.option.data.length) * (offset))}px`;
        for (let i = 0; i < this.allElements.length; i++) {
            var test = this.leftOrRight(i);
            this.allElements[i][0].className = "Timeline_Year_inline_classic";
    ...
    mode_Inline_Circle(width, offset) {
        for (let i = 0; i < this.allElements.length; i++) {
            var test = this.leftOrRight(i);
    ...
    mode_Inline(width, offset) {
        ...
        var dotEnd = document.createElement("span");
    ...
    mode_Classic(width, offset) {
        ...
        for (let i = 1; i < this.dotLocations.length; i++) {
            ...
            var path = document.createElementNS("http://www.w3.org/2000/svg", "path");
            var width = this.option.width;
            var dotEndLine = document.createElement("span");
